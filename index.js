      ...
      completed: userCompletedTasks
        ? await Promise.all(
            userCompletedTasks.map(async (completedTask) => {
              ...
              TotalTimesCompleted += completedTask.doerTimesCompleted;
              return {
                ...
                tasks: [
                  {
                    taskName: task.name,
                    timesCompleted: completedTask.doerTimesCompleted,
                  },
                ],
              };
            })
        ...
        const taskAdsData = await Promise.all(
          taskAds.map(async (adElement) => {
            const customerArrayForAd = await customers
              .find({
                customerId: userIdFromToken ? userIdFromToken : "",
              })
              .toArray();
            ...
            return {
              ...
              customerVariantPrice: adElement.customerVariantPrice,
              hasCompleted: token
